...
function header(session = false, hideTeacher = false, hideClose = false) {
  return `<header class="topbar"><div class="brand"><img class="brand-mark" src="logo.png" alt="Logo Sumatif Kelas 10"><div>SUMATIF X<small>Informatika • Kelas 10</small><small class="copyright">© Muhammad Husein Arafat 2026</small></div></div><div class="topbar-actions">${session ? `<div class="session-chip"><span class="session-dot"></span>${esc(state.student.name)}</div>` : hideTeacher ? '' : '<button class="teacher-entry" id="teacherLogin">Login Guru</button>'}${hideClose ? '' : '<button class="close-page" id="closePage" type="button">Close</button>'}</div></header>`;
}
...
function renderExam() {
  ...
  app.querySelector('.exam-sidebar .brand-mark')?.setAttribute('src', 'logo.png');
...
function renderClosing() {
  ...
  app.innerHTML = `${header(true, false, true)}<main class="result-wrap"><section class="close-note"><div class="result-icon">✓</div><h2>Perjalanan ujian selesai</h2><p>Terima kasih sudah menyelesaikan seluruh rangkaian ujian. Silakan catat nilai akhir kamu dan tunggu arahan dari guru sebelum meninggalkan halaman ini.</p><div class="score" style="color: var(--green-700); margin: 24px 0 10px;">${finalScore}<small style="color:var(--muted);"> / 100</small></div><div class="score-comparison"><div class="score-item"><strong>${state.mainScore ?? '-'}</strong><span>Nilai ulangan asli</span></div><div class="score-item"><strong>${state.remedialScore ?? '-'}</strong><span>Nilai remedi</span></div><div class="score-item"><strong>${state.enrichmentScore ?? '-'}</strong><span>Nilai pengayaan</span></div></div><p>${bonus ? 'Bonus pengayaan: +1 poin.' : 'Tidak ada bonus pengayaan.'}</p><div class="result-card"><strong>Jangan lupa catat nilai kalian masing-masing!</strong></div></section></main>`;
}
